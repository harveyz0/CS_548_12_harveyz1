...
def arg_parser(args):
    parser = ArgumentParser(prog=args[0], description='Module to build and run a pix2pix model')
    parser.add_argument('-c', '--checkpoint', action='store', help='Start from this checkpoint file')
    ...
    parser.add_argument('-l', '--load-model', action='store', help='Load a model from a directory and then try to push it through all the images passed in')
    parser.add_argument('-r', '--resize', action='store_true', help='Resize images from orig_data_directory to data_directory')
...
def load_config_arg(parser):
    ...
    if parser.checkpoint:
        cfg.checkpoint_filename = parser.checkpoint
    return cfg
...
def resize_images(parser):
    ...
    do_resize(cfg.orig_data_directory, cfg.data_directory, cfg.img_resize_height, cfg.img_resize_width)
